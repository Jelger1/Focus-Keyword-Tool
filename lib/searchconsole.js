...
/**
 * Zonder wachtwoord is een publieke deploy voor iedereen met de link bruikbaar.
 * Met Search Console erbij zou dat de zoekdata van klanten weggeven. Op een
 * publieke Vercel-deploy (production of preview) draait Search Console daarom
 * alleen als APP_PASSWORD gezet is; lokaal kan alleen de ontwikkelaar erbij.
 */
export function searchConsoleAllowed(env) {
  const publicDeployment = env.VERCEL_ENV === 'production' || env.VERCEL_ENV === 'preview';
  return !publicDeployment || Boolean(env.APP_PASSWORD);
}
...
/** Eén aanvraag voor deze pagina, met een optionele extra dimensie. */
function queryPage(client, { property, page, startDate, endDate, dimensions }, requestOptions) {
 ...
 * }>}
 */
export async function fetchPageQueries(pageUrl, { env, timeoutMs = TIMEOUT_MS, now = Date.now() } = {}) {
  const base = { attempted: false, error: false, property: null, page: null, startDate: null, endDate: null, rows: [], pageTotals: null };
  ...
  if (!searchConsoleAllowed(settings)) {
    return done(
      GSC_STATUS.nietBeveiligd,
      'Search Console staat uit: zet APP_PASSWORD in Vercel. Zonder wachtwoord kan iedereen met de link de zoekdata van klanten opvragen.'
    );
  }
  ...
  try {
    return await withTimeout((async () => {
      ...
      const range = { property, startDate, endDate };
      for (const page of pageVariants(pageUrl)) {
        ...
        return done(GSC_STATUS.ok, `${rows.length} zoekopdrachten uit Search Console (${startDate} t/m ${endDate}).${measuredOther}`, {
          attempted: true, property, page, startDate, endDate, rows, pageTotals,
        });
      }
      return done(GSC_STATUS.leeg, `Search Console heeft voor deze pagina geen vertoningen tussen ${startDate} en ${endDate}, ook niet zonder parameters of andere slash.`, {
        attempted: true, property, startDate, endDate,
      });
    })(), timeoutMs);
  } catch (error) {
