...
4. Staat er niets passends in de lijst, stel dan zelf drie tot vijf zoekwoorden voor die passen bij wat de pagina is, of hoe hij bedoeld is: concrete zoektermen in de taal van de regio, zoals mensen ze daar intypen, van specifiek naar breder. De tool controleert hun zoekvolume bij Ahrefs.

Regels:
- Kies uit de lijst alleen zoekwoorden die er letterlijk in staan; de tool controleert dat.
- Een eigen voorstel (stap 4) heeft nog geen cijfers: noem er geen zoekvolume, vertoningen of positie bij. De tool zoekt het volume op bij Ahrefs.
- Onderbouw een keuze met wat in het bericht staat: de tekst en koppen van de doelpagina en de cijfers in de lijst.
- Uitleg in het Nederlands, je-vorm, concreet; zoekwoorden in de taal van de regio.
...
export function describeRow(row, metrics, region = DEFAULT_REGION) {
  const parts = [];
  ...
  // Console, de beste positie in de regio bij Ahrefs. Zonder label vergelijkt
  ...
  if (row.position !== null && row.position !== undefined) {
    parts.push(row.origin === ROW_ORIGIN.ahrefs
      ? `beste positie ${fmtPos(row.position)} (Ahrefs, ${region.short})`
      : `gem. positie ${fmtPos(row.position)} (Search Console)`);
  }
  if (row.ahrefsPosition !== null && row.ahrefsPosition !== undefined) {
    parts.push(`beste positie ${fmtPos(row.ahrefsPosition)} (Ahrefs, ${region.short})`);
  }
...
function listExplanation(source, gsc, region) {
  const lines = [];
  if (source === DATA_SOURCE.gscUpload) {
    ...
    lines.push(`Search Console: vertoningen en klikken van ${gsc.startDate} t/m ${gsc.endDate}, ${gsc.country ? `alleen ${region.label}` : 'alle landen samen'}; gem. positie = gemiddelde over die periode.`);
  }
  if (source !== DATA_SOURCE.gscUpload && source !== DATA_SOURCE.gscOnly) {
    lines.push(`Ahrefs: volume = zoekopdrachten per maand in ${region.label}; beste positie = de hoogste positie van deze URL in ${region.label} volgens Ahrefs; bezoekers per maand is een schatting.`);
  }
  return lines;
}

export function buildRefocusMessage({ target, rejectedKeyword, intent, rows, metrics, source, gsc = null, region = DEFAULT_REGION }) {
  ...
  const lines = [
    ...
    '',
    `# ${sourceLabel}`,
    ...listExplanation(source, gsc, region),
    ...rows.slice(0, MAX_ROWS_FOR_MODEL).map((row, index) => `${index + 1}. "${row.query}": ${describeRow(row, metrics.get(normalize(row.query)), region)}`),
  ];
