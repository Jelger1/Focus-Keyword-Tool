...
export function describeSerpProvider(provider, updatedAt) {
  if (provider === 'ahrefs') {
    ...
    const stamp = date && !Number.isNaN(date.getTime())
      ...
      : '';
    return `Ahrefs (Google Nederland${stamp})`;
  }
  return 'Serper.dev (Google Nederland, live)';
}
 ...
 * }>}
 */
export async function fetchSerp(keyword, { provider, apiKey }) {
  if (provider === 'ahrefs') {
    const serp = await fetchSerpOverview(keyword, { apiKey });
    return { provider, ...serp };
  }
  const serp = await fetchSerper(keyword, { apiKey });
