...
export const STOPWORDS = new Set([
  ...
  // Engelse functiewoorden: op een Nederlandse pagina nooit een vakterm, dus ze mogen
  // in de gedeelde lijst. Engelse woorden met inhoud staan in EN_TERM_EXCLUDE.
  'about', 'above', 'after', 'again', 'also', 'any', 'because', 'been', 'before', 'being', 'below',
  'between', 'both', 'but', 'could', 'did', 'does', 'doing', 'during', 'each', 'further', 'had',
  'has', 'have', 'having', 'here', 'how', 'into', 'its', 'just', 'more', 'most', 'much', 'must', 'not',
  'now', 'off', 'once', 'only', 'other', 'out', 'own', 'same', 'should', 'some', 'such', 'than',
  'their', 'them', 'then', 'there', 'these', 'they', 'those', 'through', 'too', 'until', 'very',
  'was', 'were', 'what', 'when', 'where', 'which', 'while', 'who', 'whom', 'why', 'will', 'would',
  'yours', 'every', 'within', 'without', 'across', 'along', 'around', 'however', 'including',
  'whether', 'yet',
]);
...
/**
 * Alleen voor analyses in het Engels (regio Verenigde Staten): woorden met een
 * beetje inhoud die in het Engels toch vooral opvulling of sitetekst zijn. Ze
 * staan niet in de gedeelde lijsten, want dan vielen Nederlandse termen als
 * "landing page" of "call to action" weg. Woorden die in het Engels ook een
 * vakterm zijn (page, search, cart, checkout, site, call, click) staan er bewust niet in.
 */
export const EN_TERM_EXCLUDE = new Set([
  'best', 'new', 'first', 'good', 'great', 'help', 'know', 'learn', 'read', 'see', 'take', 'find',
  'want', 'well', 'like', 'make', 'makes', 'many', 'may', 'need', 'needs', 'get', 'gets', 'getting',
  'use', 'used', 'using', 'way', 'one', 'can', 'over', 'under', 'down', 'few',
  'accept', 'settings', 'newsletter', 'subscribe', 'signup', 'sign', 'policy', 'terms', 'conditions',
  'rights', 'reserved', 'share', 'back', 'next', 'previous', 'article', 'articles', 'comment',
  'comments', 'author', 'posted', 'published', 'updated', 'address', 'hours', 'careers', 'skip',
  'close', 'submit', 'toggle',
]);

const NOTHING = new Set();

/** De extra uitsluitingen voor een regio: Engels bij US, niets extra bij NL. */
export function extraTermExclusions(region) {
  return region?.id === 'us' ? EN_TERM_EXCLUDE : NOTHING;
}
