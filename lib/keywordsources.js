 ...
 */
export async function collectKeywordRows({ mode, pageUrl, uploadText, env, ahrefsKey, region = DEFAULT_REGION }) {
  if (mode === 'upload') {
  ...
  const [gsc, ahrefs] = await Promise.all([
    fetchPageQueries(pageUrl, { env, region }),
    ahrefsKey
      ? fetchOrganicKeywords(pageUrl, { apiKey: ahrefsKey, country: region.ahrefsCountry }).then(
  ...
  const ahrefsNote = flags.source !== DATA_SOURCE.gscOnly
    ? ''
    : ahrefs.error ? ' Ahrefs-cijfers waren niet beschikbaar.' : ` Ahrefs ziet deze URL in ${region.label} niet ranken.`;
