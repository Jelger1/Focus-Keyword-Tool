...
export const GAP_TOPICS_SYSTEM_PROMPT = `Je bent een senior SEO-strateeg. Je voert een content gap-analyse uit op échte data: de organische Google-topresultaten (Nederland) voor een zoekwoord, met per resultaat de koppen die op die pagina staan. De intent check is al gedaan: het zoekwoord past bij de pagina. Jij helpt de pagina nu beter te maken dan de concurrentie.
...
export const GAP_TOPICS_SCHEMA = {
  ...
  properties: {
    topics: {
      ...
      items: {
        ...
        properties: {
          ...
          advice: { type: 'string', description: 'Eén zin: wat je toevoegt of aanpast.' },
          sources: {
            type: 'array',
            items: {
              type: 'object',
    ...
    summary: { type: 'string', description: 'Drie tot vijf zinnen voor de marketeer.' },
  },
...
export function buildGapTopicsMessage({ keyword, target, compared, questions, intent, pageTypeOf }) {
  const lines = [
    '# Focus zoekwoord',
    keyword,
    '',
...
export const GAP_TERMS_SCHEMA = {
  ...
  properties: {
    ...
    placement: {
      type: 'object',
      properties: {
        h1: { type: 'string', description: 'Nieuwe H1, of leeg als het zoekwoord er al letterlijk in staat.' },
...
export function buildGapTermsMessage({ keyword, target, compared, candidates, intent, placement, mappingCandidates, pageTypeOf }) {
  const lines = [
    '# Focus zoekwoord',
    keyword,
    '',
  ...
  lines.push(
    ...
    '',
    '# Zoekwoordideeën (zoekwoord — volume per maand volgens Ahrefs — bron; "Search Console" = gemeten vertoningen van deze pagina)',
