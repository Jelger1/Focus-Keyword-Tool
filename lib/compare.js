 ...
 * mag kiezen: de ideeën van Ahrefs plus de topzoekwoorden van de concurrenten.
 * Het focus zoekwoord zelf hoort er niet bij.
 */
export function mappingCandidates(ideas, serp, keyword) {
  ...
  return [...byKey.values()].sort((a, b) => (b.volume ?? 0) - (a.volume ?? 0));
}
...
export function buildReport({
  ...
  const pickIdeas = (items) => {
    ...
    for (const item of Array.isArray(items) ? items : []) {
      ...
      usedIdeas.add(key);
      picked.push({ keyword: idea.keyword, volume: idea.volume, why: String(item.why || ''), sources: idea.sources });
    }
    return picked;
  };
  const mapping = {
