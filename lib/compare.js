 ...
 *
 * Verdeling van het werk: de code meet (woorden, koppen, termfrequenties) en
 * controleert; Claude groepeert en formuleert. Alles wat Claude aanlevert, wordt
 * hier tegen de gemeten data gehouden — een onderwerp zonder aantoonbare bron of
 * een term die niet geteld is, haalt het rapport niet.
 */
 ...
 */
export function buildReport({ keyword, target, serp, serpResults, compared, candidates, questions, model, provider }) {
  ...
  return {
    ...
    serp: {
      ...
      compared: total,
      // Serper levert het "Mensen vragen ook"-blok niet voor elke zoekopdracht; voor
      // Nederlandse zoekwoorden bleef het in tests leeg. De frontend meldt dat dan.
      peopleAlsoAsk: serp.peopleAlsoAsk.length,
      results: serpResults.map(({ page, ...result }) => result),
    ...
    intent: { summary: model.searchIntent, type: model.intentType },
    coverage: {
      score: coverageScore,
    ...
    quality: { droppedTopics },
    disclaimer:
      `Vergeleken met ${total} pagina's uit de Google-top 10 (${provider}). Woordenaantallen, koppen, ` +
      'termen en "Mensen vragen ook" zijn gemeten. Het groeperen van koppen tot onderwerpen, de voorgestelde ' +
      'formulering en de zoekintentie zijn interpretaties van Claude; elk onderwerp is gecontroleerd op ' +
      'letterlijke koppen bij minstens twee concurrenten.',
  };
}
