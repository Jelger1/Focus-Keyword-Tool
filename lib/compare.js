...
import {
  normalize, stem, contentWords, stemmedHaystack, wordOnPage, STOPWORDS, BOILERPLATE, extraTermExclusions,
} from './text.js';
...
/**
 * Engelse sitekoppen. Specifiek en waar nodig aan het begin verankerd: een los
 * 'related' zou ook 'Related costs of packaging' wegfilteren.
 */
const BOILERPLATE_HEADING_EN = /newsletter|subscribe|sign up for|follow us|share this|^related (articles|products|posts|content)\b|you may also like|^read (more|also)\b|most (read|popular)|shopping cart|\bmy account\b|^(log|sign) in\b|^about us\b|customer (service|support)|get in touch|^need (help|advice)\b|^(have|any|more) questions|^interested in (this|these|our)\b|can'?t find|looking for something/i;
 ...
 */
export async function fetchCompetitors(organic, target, { acceptLanguage } = {}) {
  ...
  return Promise.all(
    organic.map(async (result) => {
      ...
      try {
        const page = await fetchPage(result.url, { timeoutMs: COMPETITOR_TIMEOUT_MS, acceptLanguage });
        return {
          ...base,
          status: 'vergeleken',
...
export function topicHeadings(page) {
  return page.headings
    ...
    .filter((heading) => !BOILERPLATE_HEADING.test(heading.text) && !BOILERPLATE_HEADING_EN.test(heading.text.trim()))
...
function isTermWord(word, extra) {
  return word.length >= 3 && !/^\d+$/.test(word) && !STOPWORDS.has(word) && !BOILERPLATE.has(word) && !extra.has(word);
}
...
function pageGrams(text, extra) {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    const word = tokens[i];
    if (!isTermWord(word, extra)) continue;
    if (word.length >= 5) add([word]);
    const next = tokens[i + 1];
    if (next && isTermWord(next, extra)) add([word, next]);
  }
  return grams;
}
 ...
 */
export function termCandidates(competitorPages, targetPage, keyword, { region } = {}) {
  const extra = extraTermExclusions(region);
  const counts = new Map(); // sleutel -> { df, forms: Map<vorm, aantal> }
  for (const page of competitorPages) {
    for (const [key, form] of pageGrams(`${page.title} ${page.text}`, extra)) {
