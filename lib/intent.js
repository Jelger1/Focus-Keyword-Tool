 ...
 */
function searchConsoleLines(gscInsight, gsc) {
  if (!gscInsight) {
    return ['# Search Console van de doelpagina', `Niet beschikbaar (${gsc?.message || 'niet gekoppeld'}).`];
  }
  ...
  return [
    `# Search Console van de doelpagina (meting van Google, ${gsc.startDate} t/m ${gsc.endDate}, alle landen samen)`,
...
export function buildIntentMessage({ keyword, target, serp, serpResults, keywordInfo, measured, providerLabel, gsc = null, gscInsight = null }) {
  ...
  lines.push(
    ...
    '',
    ...searchConsoleLines(gscInsight, gsc),
    '',
